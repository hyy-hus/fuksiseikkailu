...
def bulk_create_teams(
    ...
    for payload in teams:
        team = DBTeam(name=payload.name, number=payload.number, adventure_id=adventure_id)
...
@router.post(
    ...
    operation_id="importTeams",
    summary="Update / Add many teams at once",
    description="Adds or updates teams in bulk",
    responses={
        200: {"description": "All imported successfully"},
...
def import_teams(
    ...
    identifiers = payload.identifiers or ["name"]  # default to name if not provided
    ...
    # Figure out allowed fields on DBTeam (pydantic v2 / v1 compatible)
    model_fields = getattr(DBTeam, "model_fields", None)
    all_fields = set(model_fields.keys()) if model_fields else set(getattr(DBTeam, "__fields__", {}).keys())
    forbidden = {"id", "adventure_id", "created_at", "updated_at"}
    allowed_update_fields = all_fields - forbidden

    for row in rows:
        # --- LOOKUP ---
        try:
            # always scope by adventure_id
            query = select(DBTeam).where(DBTeam.adventure_id == adventure_id)

            for col in identifiers:
                if col not in all_fields:
                    errors.append({"error": f"Unknown identifier field: {col}", "row": row})
                    query = None  # skip this row
                    break
                query = query.where(getattr(DBTeam, col) == row.get(col))
            ...
            errors.append({"type": "lookup_error", "error": f"Failed lookup: {str(e)}", "row": row})
            continue

        # --- CREATE ---
        if not db_team:
            try:
                create_payload = {**row, "adventure_id": adventure_id}
                team_data = CreateTeam(**create_payload).model_dump(exclude_unset=False)
                db_team = DBTeam.model_validate(team_data)
                session.add(db_team)
                created_count += 1
        ...
        # --- UPDATE (partial) ---
            ...
            try:
                update_data = ModifyTeam(**row).model_dump(exclude_unset=True)

                # keep identifier fields as-is (no rename via import), and drop restricted ones
                update_data.pop("id", None)
                update_data.pop("adventure_id", None)
                for col in identifiers:
                    update_data.pop(col, None)
                # ignore players entirely during this import
                ...
                errors.append({"type": "update_validation_error", "error": str(e), "row": row})
                continue
            except Exception as e:
                errors.append({"type": "update_error", "error": f"Failed to update: {str(e)}", "row": row})
                continue

    session.commit()

    # Keep your original behavior (400 on any errors), or switch to 207 if you prefer partial success.
    if errors:
...
@admin_router.delete(
    ...
    description="Delete teams in an adventure",
    responses={
        200: {"description": "Teams deleted succesfully"},
        404: {"description": "Teams could not be found"},
    },
)
def delete_teams(
    user: UserDep,
    ...
    result = session.exec(delete(DBTeam).where(DBTeam.adventure_id == adventure_id))
    session.commit()
    return DeleteResponse(ok=True, deleted=result.rowcount or 0)
